...
@Component({
  tag: 'lmd-sidebar-element',
  styleUrl: 'lmd-sidebar-element.css',
  shadow: true,
})
export class LmdSidebarElement {
            ...
            {this.expanded && Array.from(subNodes).map(([path, id]) => {
              return (<div><span class="sidebar-propertyname">{path.split("/").pop()}:</span>
                <div class="sidebar-properties">
                  {id.map((id) =>
                    <lmd-sidebar-element data={this.data} selector={jsonLdGetByID(this.data, id)} globalSelector={this.globalSelector}></lmd-sidebar-element>
                  )}
                </div>
              </div>);
              }
